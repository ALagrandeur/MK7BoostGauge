...
# Windows doesn't have ssh-copy-id, so do it manually via ssh
$pubKey = Get-Content "$KeyPath.pub" -Raw
$pubKey = $pubKey.Trim()

# Use bash-style command via ssh to append key
...
if ($test -eq "pi") {
    ...
    Write-Host "    You can still use update_pi.ps1, just enter password manually each step." -ForegroundColor Yellow
}
